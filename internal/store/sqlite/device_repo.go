...
func (r *DeviceRepo) Create(ctx context.Context, d *models.Device) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO devices (id, hostname, ip_addresses, mac_addresses, os, status, discovery_method, first_seen_at, last_seen_at, tags, ports, group_id, metadata, map_x, map_y, network_id)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		d.ID, d.Hostname, string(ips), string(macs), d.OS, d.Status, d.DiscoveryMethod,
		d.FirstSeenAt, d.LastSeenAt, string(tags), string(ports), d.GroupID, string(meta), d.MapX, d.MapY, d.NetworkID,
	)
	return err
}
...
func (r *DeviceRepo) GetByID(ctx context.Context, id string) (*models.Device, error) {
	return r.scanDevice(r.db.QueryRowContext(ctx,
		`SELECT id, hostname, ip_addresses, mac_addresses, os, status, discovery_method, first_seen_at, last_seen_at, tags, ports, group_id, metadata, map_x, map_y, network_id FROM devices WHERE id = ?`, id))
}

func (r *DeviceRepo) GetByMAC(ctx context.Context, mac string) (*models.Device, error) {
	return r.scanDevice(r.db.QueryRowContext(ctx,
		`SELECT id, hostname, ip_addresses, mac_addresses, os, status, discovery_method, first_seen_at, last_seen_at, tags, ports, group_id, metadata, map_x, map_y, network_id FROM devices WHERE mac_addresses LIKE ?`,
		fmt.Sprintf("%%%s%%", mac)))
}

func (r *DeviceRepo) GetByHostname(ctx context.Context, hostname string) (*models.Device, error) {
	return r.scanDevice(r.db.QueryRowContext(ctx,
		`SELECT id, hostname, ip_addresses, mac_addresses, os, status, discovery_method, first_seen_at, last_seen_at, tags, ports, group_id, metadata, map_x, map_y, network_id FROM devices WHERE hostname = ?`, hostname))
}

func (r *DeviceRepo) GetByIP(ctx context.Context, ip string) (*models.Device, error) {
	return r.scanDevice(r.db.QueryRowContext(ctx,
		`SELECT id, hostname, ip_addresses, mac_addresses, os, status, discovery_method, first_seen_at, last_seen_at, tags, ports, group_id, metadata, map_x, map_y, network_id FROM devices WHERE ip_addresses LIKE ?`,
		fmt.Sprintf("%%%s%%", ip)))
}
...
func (r *DeviceRepo) scanDevice(row *sql.Row) (*models.Device, error) {
	...
	err := row.Scan(&d.ID, &d.Hostname, &ips, &macs, &d.OS, &d.Status, &d.DiscoveryMethod,
		&d.FirstSeenAt, &d.LastSeenAt, &tags, &ports, &d.GroupID, &meta, &d.MapX, &d.MapY, &d.NetworkID)
...
func (r *DeviceRepo) List(ctx context.Context, params models.ListParams) (*models.ListResult[models.Device], error) {
	...
	query := fmt.Sprintf("SELECT id, hostname, ip_addresses, mac_addresses, os, status, discovery_method, first_seen_at, last_seen_at, tags, ports, group_id, metadata, map_x, map_y, network_id FROM devices WHERE %s ORDER BY %s %s LIMIT ? OFFSET ?",
	...
	for rows.Next() {
		...
		if err := rows.Scan(&d.ID, &d.Hostname, &ips, &macs, &d.OS, &d.Status, &d.DiscoveryMethod,
			&d.FirstSeenAt, &d.LastSeenAt, &tags, &ports, &d.GroupID, &meta, &d.MapX, &d.MapY, &d.NetworkID); err != nil {
			return nil, err
		}
		if err := json.Unmarshal([]byte(ips), &d.IPAddresses); err != nil {
			return nil, fmt.Errorf("unmarshal ip_addresses: %w", err)
...
func (r *DeviceRepo) Update(ctx context.Context, d *models.Device) error {
	...
	_, err := r.db.ExecContext(ctx,
		`UPDATE devices SET hostname=?, ip_addresses=?, mac_addresses=?, os=?, status=?, last_seen_at=?, tags=?, ports=?, group_id=?, metadata=?, map_x=?, map_y=?, network_id=? WHERE id=?`,
		d.Hostname, string(ips), string(macs), d.OS, d.Status, d.LastSeenAt,
		string(tags), string(ports), d.GroupID, string(meta), d.MapX, d.MapY, d.NetworkID, d.ID,
	)
	return err
}
